...
Three operational improvements: single binary merge, `ephyr init` wizard, and RISK-3 proxy hardening.

### Added

- **Single binary (`ephyr`)** -- broker, signer, and all CLI tools merged into one binary. `ephyr broker` and `ephyr signer` replace the standalone `ephyr-broker` and `ephyr-signer` binaries. Legacy binaries are still built by `make build` for backward compatibility.
- **`ephyr init` wizard** -- one-command setup that generates the CA key, writes an example policy, creates the system user and directories, installs systemd units, and starts both services. `--dev` flag for development defaults.
- **`ephyr version` subcommand** -- prints version, Go version, and build info.
- **Proxy hardening (RISK-3)** -- hop-by-hop header stripping per RFC 2616, `HTTP_PROXY`/`http_proxy` sanitized at startup (httpoxy/CVE-2016-5385 mitigation), redirect following disabled (3xx returned to agent), credential-bearing response headers (`Authorization`, `Set-Cookie`, `Cookie`, `Proxy-Authorization`) stripped from proxied responses, `X-Ephyr-Proxy: true` header added to all proxied requests.

### Changed

- **`cmd/ephyr/` is the primary entry point** -- `cmd/broker/` and `cmd/signer/` are retained as thin wrappers for backward compatibility.
- **Systemd units use `ephyr broker` / `ephyr signer`** -- generated by `ephyr init`. Existing units referencing `ephyr-broker` / `ephyr-signer` continue to work.
