...
func cmdBroker(args []string) {
	...
	var (
		policyPath      = fs.String("policy", envOrDefault("EPHYR_POLICY", "/etc/ephyr/policy.yaml"), "path to policy YAML file")
		signerSocket    = fs.String("signer-socket", envOrDefault("EPHYR_SIGNER_SOCKET", "/run/ephyr/signer.sock"), "path to signer IPC socket")
		listenSocket    = fs.String("listen", envOrDefault("EPHYR_LISTEN", "/run/ephyr/broker.sock"), "path for broker Unix socket")
		auditLogPath    = fs.String("audit-log", envOrDefault("EPHYR_AUDIT_LOG", "/var/log/ephyr/audit.json"), "path to audit log file")
		dashboardListen = fs.String("dashboard-listen", envOrDefault("EPHYR_DASHBOARD_LISTEN", ":8553"), "TCP address for dashboard listener")
		dashboardToken  = fs.String("dashboard-token", envOrDefault("EPHYR_DASHBOARD_TOKEN", ""), "API token for dashboard (auto-generated if empty)")
		socketGroup     = fs.String("socket-group", envOrDefault("EPHYR_SOCKET_GROUP", "ephyr-agents"), "Group for broker socket ownership")
		dashboardDir    = fs.String("dashboard-dir", envOrDefault("EPHYR_DASHBOARD_DIR", "/opt/ephyr/dashboard"), "directory for static dashboard files")
		mcpListen       = fs.String("mcp-listen", envOrDefault("EPHYR_MCP_LISTEN", ":8554"), "TCP address for MCP listener")
	)
	...
	fs.Var(&adminUIDs, "admin-uid", "UID allowed admin operations (can be repeated)")
	fs.Parse(args)
